...
export function ArticleCard({ article, isCompact = false }: ArticleCardProps) {
  // Function untuk membersihkan summary dari "[+XXXX chars]"
      ...
      <div className={`p-4 ${isCompact ? 'w-2/3' : ''}`}>
        ...
        {!isCompact && displaySummary && (
          <div>
            ...
            {article.isExternal && (isTruncated(article.summary || '') || displaySummary.length < 100) && (
        ...
        <div className={`flex items-center justify-between text-sm text-gray-500 ${isCompact ? 'mt-1' : 'mt-3'}`}>
          <div className="flex items-center gap-2">
            ...
            {article.isExternal && article.source && (
              <span className="text-blue-600 text-xs">• via {article.source}</span>
            )}
          </div>
